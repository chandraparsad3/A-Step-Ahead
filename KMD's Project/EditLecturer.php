<?php 
    ...
    if (isset($_POST['btnupdate'])) {
        ...
        $image=$_FILES["txtPradeep"]['name'];
        $folder="images/";
        if($image)
        {
         $filename=$folder.$lid."_".$image;
         $copied=copy($_FILES['txtPradeep']['tmp_name'],$filename);
         if(!$copied)
         {
        ...
        elseif ($result) {
            echo "<script>window.alert('Lecturer Update Successful');</script>";
            echo "<script>window.location='LecturerEntry.php';</script>";
        }
        else{
            echo "<script>window.alert('Lecturer Record Cannot Update');</script>";
            echo "<script>window.location='LecturerEntry.php';</script>";
        }
    }
    if (isset($_REQUEST['LecturerID'])) {
        ...
        echo "<script>window.alert('Please Choose Lecturer ID.');</script>";
        echo "<script>window.location='LecturerEntry.php';</script>";
    }
 ?>
<!DOCTYPE html>
...
<body>
  ...
  <div class="container-fluid page-body-wrapper">
    ...
    <form action="EditLecturer.php" method="POST" enctype="multipart/form-data">
        <table align="center" cellpadding="5">
                <tr>
                    <td>Lecturer ID</td>
                    <td><input type="text" name="txtLecturerID" value="<?php echo $arr['LecturerID']; ?>"  readonly required></td>
                </tr>
                <tr>
                    <td>Name:</td>
                ...
                <tr>
                   <td>Path:</td>
                   <td>
                     <input type="file" name='txtPradeep' required>
                   </td>
                </tr>                
                <tr>
                    <td>Phone:</td>
